...
class TestPerfisNoJWT:
    ...
    async def test_login_token_inclui_perfis(self, db_clean, admin_user):
        ...
        assert "administrador_geral" in payload["perfis"]


class TestPerfisNoUsuarioRead:
    """Endpoints de usuario devem retornar campo perfis"""

    async def test_registro_retorna_solicitacao(self, db_clean):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            r = await ac.post(
    ...
    async def test_me_retorna_perfis(self, db_clean, admin_user):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
