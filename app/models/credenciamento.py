...
class SolicitacaoCredenciamento(Base):
    ...
    avaliado_por: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("lms.usuarios.id", ondelete="SET NULL"), nullable=True
    )
    ...
    usuario: Mapped["Usuario"] = relationship(
        "Usuario",
        back_populates="solicitacoes_credenciamento",
        foreign_keys=[usuario_id],
    )
    aprovacoes: Mapped[list["AprovacaoHierarquica"]] = relationship(
        "AprovacaoHierarquica", back_populates="solicitacao", cascade="all, delete-orphan"
    )
...
class AprovacaoHierarquica(Base):
    ...
    solicitacao_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("lms.solicitacoes_credenciamento.id", ondelete="CASCADE"), nullable=False
    )
    ...
    # Relacionamentos
    solicitacao: Mapped["SolicitacaoCredenciamento"] = relationship(
        "SolicitacaoCredenciamento", back_populates="aprovacoes"
    )
